...
func Activation(c *gin.Context) {
	...
	logger.LogInfo("Activation completed successfully")
...
func SendActivationRequest(code string, requestData models.SActivateRequest) error {
	...
	url := fmt.Sprintf("http://localhost:8381/actions/api/v1/add/server/join/%s", code)
	...
	logger.LogInfo("Activation request successful")
	return nil
}
